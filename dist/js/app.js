...
spa.router = function () {
	...
	initVideosPage = function initVideosPage() {
	...
	showHomepage = function showHomepage() {
		...
		var html = spa.template.parseTemplate('features.homepage.homepage', { url: 'images/landscape.jpg' });
		$('#main-content').html(html);
...
spa.data = function () {
	var _url, _timeout, initModule, getJsonData;
...
spa.widget = function () {
	var initModule, toggle, _id, configModule;

	initModule = function initModule(wrapperId) {
		_id = wrapperId;
		$(_id).addClass('widget-error');
		$(_id).find('#widget-close').click(function () {
			toggle();
		});
	};

	configModule = function configModule() {
